  ...
  try{
    ...
    socket.ev.on('messages.upsert', async ({messages}) => {
      const m = messages[0];
        ...
        const quoted = m.message?.extendedTextMessage?.contextInfo?.quotedMessage?.conversation ? m.message?.extendedTextMessage?.contextInfo?.quotedMessage?.conversation : m.message?.extendedTextMessage?.contextInfo.quotedMessage?.imageMessage?.caption
        ...
        if (Array.isArray(plugins[command])) {
            ...
            const isOwner = plugins[command].isOwner ? plugins[command].isOwner : false;
            ...
            if (isOwner === true) {
              const owner = '6282125388500@s.whatsapp.net'
              let sender = from.endsWith('@g.us') ? m.key.participant : from
              if (sender != owner) {
                socket.sendMessage(from, {text :'Fitur Ini Hanya Bisa Oleh Owner'}, {quoted: m})
                return
              }
            }
            // console.log(from)
...
//                 socket.sendMessage(
//                     from,
//                     { image: { url: "./thumb.jpg" }, mimeType: "image/png", caption: 'Hallo', mentions: ['6282125388500@s.whatsapp.net'] },
//                     { quoted: m }
//                 );
//                 break;
//         }
